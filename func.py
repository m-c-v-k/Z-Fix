#! python3
# func.py
# A function module containing all functions used in main.py
...
path = str(os.getcwd() + str(os.path.join('\\' + 'z_fix', 'files\\')))
...
def z_fix():
    ...
    desired_measure = TextInput(
        hint_text='Laserhöjd', size_hint=(.5, .1))
...
def test_measure(desired_measure):
    ''' Checks if the user input is formatted correctly.
    ...
    measure_test = re.compile(r"\d+[\.|,]\d{1}")
    mo = re.fullmatch(cordinate_test, desired_measure)

    if mo is None:
        return 'error'

    if ',' in desired_measure:
        desired_measure = desired_measure.replace(',', '.')
